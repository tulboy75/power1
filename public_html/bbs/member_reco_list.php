...
// 현재 사용자를 추천한 회원 리스트 
if($mb_search_nick)
    $sql = "select * from " . $g5['member_table'] . " where mb_recommend = '" . $member['mb_10'] . "' AND mb_nick = '" . $mb_search_nick . "' order by mb_no desc limit 1";
else
    $sql = "select * from " . $g5['member_table'] . " where mb_recommend = '" . $member['mb_10'] . "' order by mb_no desc";
